...
def generate_html(results: List[Tuple[GameData, float, float, float]]) -> str:
    ...
    def generate_most_top_prizes_rows(games, limit=10):
        """Generate rows for games ranked by most top prizes remaining"""
        # Sort by top prizes remaining (desc), then by differential (desc) as tiebreaker
        sorted_games = sorted(games, key=lambda x: (x[0].get_top_prize().remaining, x[3]), reverse=True)
...
<body>
    <div class="container">
        ...
        <section>
            <div class="section-header">
                <h2 class="section-title"><span class="icon">&#127942;</span> Most Top Prizes Remaining</h2>
                <span class="section-count">all prices</span>
            </div>
            <p style="color: var(--text-secondary); margin-bottom: 1rem; font-size: 0.9rem;">Games ranked by the number of top prizes still available. Differential is used as a tiebreaker.</p>
            <div class="table-container">
                <table>
                    <thead>
                        <tr>
